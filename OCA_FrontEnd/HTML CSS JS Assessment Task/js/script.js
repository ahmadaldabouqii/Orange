...
function validate() {
  if (delivery_time.value === '4:00pm' || delivery_time.value === '') {
    ...
    delivery_time
      .closest('.form-group')
      .querySelector('.invalid-feedback').innerHTML = `Unfortunately ${
      type.value
    } cannot be delivered at ${
      delivery_time.value === '' ? 'at this time' : delivery_time.value
    }`;
  } else {
  ...
  if (
    (type.value === 'chocolate' && allergies.value === 'dairy_free') ||
    (type.value === 'pecan' && allergies.value === 'nut_free')
  ) {
    ...
    type
      ...
      .querySelector(
        '.invalid-feedback'
      ).innerHTML = `you can't take  ${type.value} if you have ${allergies.value}`;
  } else {
...
function setDisplay(forWhat, cls, type) {
  forWhat
    .closest('.form-group')
    .querySelector(`.${cls}`).style.display = `${type}`;
}
...
document.querySelector('.btn-info').addEventListener('click', () => {
  ...
  if (retrieveUsers) {
    ...
    document.querySelector('#welcome').innerHTML = `Welcome ${
      retrieveUsers[retrieveUsers.length - 1].name
    }`;
  }
});
